---
title: "Chapter 4 Exercises: Brittani and Zach"
...
```{r}
cylmpg = summarize(group_by(mycars,cyl), avgmpg = mean(mpg), count = n())
cylmpg
```
```{r}
ggplot(data = cylmpg, mapping = aes(fill=cyl, x=avgmpg)) + geom_bar()
```
...
```{r}
carps <- mutate(mtcars, powerspeed = hp/qsec)
carps
```

```{r}
carps1 = summarize(group_by(carps,powerspeed), count = n())
carps1
```
